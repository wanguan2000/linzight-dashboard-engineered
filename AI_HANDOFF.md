...
- 登录页与 Demo 账号认证回退；登录页支持 `Study 研究入口` 和 `LZ 系统管理`。Study 入口先认证账号，账号只授权一个 Study 时直接进入该 Study Workspace，授权多个 Study 时认证后再选择工作区；LZ 管理页不是业务租户，仅作为全局配置和索引入口。
...
- Demo seed：`backend/seed.py` 读取 CRF V0.1 schema，生成 70 名测试患者、210 条访视、140 条随访记录、210 条 CRF 记录及关联样本/组学/知情同意数据；CRF payload 在 SQLite 中以 JSONB BLOB 优先保存，在 PostgreSQL runtime 中以兼容 JSON 文本保存，并带版本信息。
...
- 完整生产级认证、集中身份源、权限审批流和企业级用户管理；当前已有 Demo 级用户资料修改、Study 成员管理、Study 系统管理员分配和平台角色 Study scope 管理。
...
- 后端已替换为本地签名 Bearer token、PBKDF2 密码哈希和账号启用/禁用控制；当前数据库层仍是 Demo 级实现，不能直接承载真实患者隐私数据。
...
- 当前隔离模型已按 Demo API smoke 覆盖管理员全局患者索引、Study CRC 跨 Study 403、样本/检测/知情同意/文件/Query/质量/导出/审批/审计列表不串 Study；浏览器矩阵覆盖桌面和 390px 移动端主路由。当前 Demo 阶段先用应用层过滤，生产 PostgreSQL 版本仍需补 RLS、真实身份源和更多浏览器覆盖。
