...
LUNG_CRF_FIELDS = [
    ...
    {"id": "LUNG-003", "name": "肿瘤类型", "sourceName": "肿瘤类型", "sourceColumn": 903, "type": "select"},
    {"id": "LUNG-004", "name": "肿瘤分期", "sourceName": "肿瘤分期", "sourceColumn": 904, "type": "select"},
    {"id": "LUNG-005", "name": "ECOG评分", "sourceName": "ECOG评分", "sourceColumn": 905, "type": "number"},
    {"id": "LUNG-006", "name": "驱动基因", "sourceName": "驱动基因", "sourceColumn": 906, "type": "text"},
    {"id": "LUNG-007", "name": "初始治疗方案", "sourceName": "初始治疗方案", "sourceColumn": 907, "type": "text"},
    {"id": "LUNG-008", "name": "耐药机制", "sourceName": "耐药机制", "sourceColumn": 908, "type": "text"},
    {"id": "LUNG-009", "name": "RECIST疗效", "sourceName": "RECIST疗效", "sourceColumn": 909, "type": "select"},
    {"id": "LUNG-010", "name": "PFS（月）", "sourceName": "PFS（月）", "sourceColumn": 910, "type": "number"},
    {"id": "LUNG-011", "name": "ctDNA突变丰度", "sourceName": "ctDNA突变丰度", "sourceColumn": 911, "type": "text"},
    {"id": "LUNG-012", "name": "转移部位", "sourceName": "转移部位", "sourceColumn": 912, "type": "text"},
    {"id": "LUNG-013", "name": "标本类型", "sourceName": "标本类型", "sourceColumn": 913, "type": "text"},
    {"id": "LUNG-014", "name": "检测项目", "sourceName": "检测项目", "sourceColumn": 914, "type": "text"},
    {"id": "LUNG-015", "name": "当前治疗线数", "sourceName": "当前治疗线数", "sourceColumn": 915, "type": "number"},
]


def lung_crf_schema() -> dict[str, Any]:
    sections = [
        *SLE_CRF_SCHEMA["sections"],
        {"id": "lung-resistance", "title": "肺癌耐药研究字段", "fields": LUNG_CRF_FIELDS},
    ]
    return {
        **SLE_CRF_SCHEMA,
        "version": "V1.0",
        ...
        "name": "真实世界肺癌耐药研究 CRF",
        "fieldCount": SLE_CRF_SCHEMA.get("fieldCount", len(SLE_CRF_FIELDS)) + len(LUNG_CRF_FIELDS),
        "sections": sections,
        "notes": [
            *SLE_CRF_SCHEMA.get("notes", []),
...
def lung_resistance_context(index: int, disease: str, organs: list[str]) -> dict[str, object]:
    ...
    return {
        ...
        "转移部位": "、".join([organ for organ in organs if organ != "肺"]) or "未见远处转移",
        "标本类型": "血液、组织" if index % 2 else "血液、组织、胸水",
...
def clinical_data(index: int, name: str, hospital_no: str, sex: str, age: int, disease: str, organs: list[str], completeness: int) -> dict[str, object]:
    ...
    payload.update(seed)
    if is_lung_resistance_disease(disease):
        payload.update(lung_resistance_context(index, disease, organs))
    payload["CRF版本"] = "V1.0" if is_lung_resistance_disease(disease) else SLE_CRF_VERSION
        ...
        for visit_index, visit_plan in enumerate(visit_plans_for_study(study_id)):
            ...
            visits.append(
                (
                    ...
                    str(visit_date),
                    visit_type,
                    str(max(0, int(visit_payload["SLEDAI评分"]) - visit_index * 2)),
        ...
        conn.executemany(
            ...
            [
                ...
                (
                    ...
                    "Study-specific lung cancer resistance CRF version with 15 appended fields.",
                    "USR-012",
                    now,
                    now,
                    now,
                ...
                encode_json(
                    {
                        ...
                        "lzxk_01_crf_fields": len(SLE_CRF_FIELDS) + len(LUNG_CRF_FIELDS),
                    }
                ),
                "127.0.0.1",
                now,
