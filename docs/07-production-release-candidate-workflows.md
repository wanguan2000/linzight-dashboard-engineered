...
本轮目标是把当前工程化 Demo 收口到内部试点 release candidate：客户可在受控环境中用真实业务场景验证流程、角色、Study 配置和数据闭环，但不可直接承载未经合规审批的真实患者生产数据。真实患者生产上线仍必须另行完成生产数据库、集中身份源、生产文件存储、安全审计、备份恢复和 UAT 签字。锁库和脱敏审批先排除在本轮客户演示正式发布范围外，仅保留为内部治理能力。
