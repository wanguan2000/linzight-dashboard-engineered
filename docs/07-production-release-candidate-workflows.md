...
| 2. 权限矩阵产品化 | 平台角色、Study 角色、菜单、按钮、tooltip、后端 403 和 API smoke 一致 | `npm run smoke:api` 覆盖 patients、crf、samples、omics、consents、files、queries、quality、exports、approvals、audit 的正向和越权 |
| 3. Query / 质控 / 访视窗口闭环 | Query 创建、回复、关闭、重开、质控一键生成 Query、访视窗口和漏访原因 | API smoke 必须覆盖 Query 状态流转、访视窗口问题生成和 Study 字段校验 |
| 4. eConsent 正式化 | 签署、撤回、重签、审批、归档、扫描件和模板按 Study 绑定 | 撤回/重签必须走 Approval Center，Approval complete 后 consent 状态刷新并写入审计 |
...
| 6. 审计与合规底座 | 写操作 audit_logs、标准 diff、登录/导出/下载/上传/审批/CRF/Query 留痕 | 任一关键操作能按 Study、实体、操作者、时间查到 before/after |
