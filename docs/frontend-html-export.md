...
- 后端连接：正式数据状态以 PostgreSQL 后端返回为准；后端不可用时，静态页面只能作为开发预览，不应作为正式数据状态判断。
- 测试 fixture：如显式运行测试 seed，页面会按用户的 Study scope 过滤数据；系统管理页包含 Study 权限矩阵、CRF 版本语义和 Study 访视计划配置展示。
