...
| Study Configuration | 更新当前 Study 配置 | `PUT /studies/{study_id}/configuration` | `LZ_ADMIN` 与本 Study `STUDY_CONFIG_ADMIN` 可写，仍按 Study scope 校验 |
...
| Patient Cohort | 新增/修改/删除患者 | `POST /studies/{study_id}/patients`, `PUT /patients/{patient_id}`, `DELETE /patients/{patient_id}` | `LZ_ADMIN`, `LZ_CRC`, `STUDY_CRC`, `STUDY_CONFIG_ADMIN` |
...
| Clinical Data Capture | 写入 CRF | `POST /studies/{study_id}/crf`, `PUT /crf/{entry_id}` | `LZ_ADMIN`, `LZ_CRC`, `STUDY_CRC`, `STUDY_CONFIG_ADMIN` |
| Clinical Data Capture | 写入随访记录 | `POST /studies/{study_id}/follow-up-records`, `PUT /follow-up-records/{record_id}` | `LZ_ADMIN`, `LZ_CRC`, `STUDY_CRC`, `STUDY_CONFIG_ADMIN` |
...
| Informed Consent | 更新知情同意/发起撤回重签 | `PUT /consents/{consent_id}`, `POST /consents/{consent_id}/withdrawal-request`, `POST /consents/{consent_id}/resign-request` | `LZ_ADMIN`, `LZ_CRC`, `STUDY_CRC`, `STUDY_CONFIG_ADMIN` |
| Samples and Testing | 写入样本 | `POST /studies/{study_id}/samples`, `PUT /samples/{sample_id}` | `LZ_ADMIN`, `LZ_CRC`, `STUDY_CRC`, `STUDY_CONFIG_ADMIN` |
| Samples and Testing | 写入组学检测 | `POST /studies/{study_id}/omics`, `PUT /omics/{omics_id}` | `LZ_ADMIN`, `LZ_CRC`, `STUDY_CRC`, `STUDY_CONFIG_ADMIN` |
| Files | 上传/下载/归档文件 | `POST /files`, `GET /files/{file_id}/download`, `POST /files/{file_id}/archive` | 写入：`LZ_ADMIN`, `LZ_CRC`, `STUDY_CRC`, `STUDY_CONFIG_ADMIN`; 下载按文件 Study scope 和扫描状态校验 |
