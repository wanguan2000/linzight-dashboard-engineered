...
- 正式 PostgreSQL runtime 当前写入兼容 JSON 文本；显式 SQLite 测试库优先写入 `patients.clinical_data_jsonb` 与 `crf_entries.payload_jsonb` BLOB；API mapper 解码后返回 JSON object，并通过 `clinical_data_version`、`clinical_data_format`、`payload_version`、`payload_format` 暴露版本和存储格式
