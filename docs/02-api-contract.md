...
| Study | `POST` | `/studies` | `LZ_ADMIN` 新建 Study，默认先进入 draft 或 active 生命周期状态；`code` 可省略，由后端分配 `01`-`99` 两位 Study Code |
...
| 患者列表 | `GET` | `/studies/{study_id}/patients` | 搜索、筛选单个 Study 患者；默认按 `created_at DESC` 返回最新建立患者 |
| 患者创建 | `POST` | `/studies/{study_id}/patients` | 在单个 Study Workspace 内创建患者；`patient_number` 由后端自动生成，范围 `H00010`-`H99999`，前端传入值会被忽略 |
...
| 样本登记 | `POST` | `/studies/{study_id}/samples` | 新增样本；`id` 由后端自动生成，前端传入值会被忽略 |
...
| 多组学检测 | `omics_records.sample_id`, `sample_ids`, `sample_usage`, `testing_project_id`, `assay`, `vendor`, `platform`, `status`, `sent_at`, `completed_at`, `qc`, `result_file_id` | `sample_id` 是兼容旧逻辑的主样本；`sample_ids` 支持一次检测人工选择多个样本；`sample_usage` 记录每个样本的送样量 `usedQuantity`、返还量 `returnedQuantity`、单位和用途；`vendor` 保存检测供应商；`result_file_id` 指向 `uploaded_files.id` 的结果文件记录。 |

样本更新时，`study_id` 跟随 `patient_id` 所属 Study，`id` 不允许修改。已有 `omics_records` 引用的样本不能直接更换患者；如需更正，应先处理关联检测记录，避免检测记录中的 `sample_ids` 出现跨患者或跨 Study 组合。
...
Study 生命周期使用 `POST /studies`、`PATCH /studies/{study_id}` 和 `DELETE /studies/{study_id}`。Study 主数据字段包括 `id`、`code`、`name`、`indication`、`phase`、`status`、`owner_org`、`leading_pi_info` 和 `system_admin`。`id` 是路由、权限和数据隔离用的稳定 Study ID；`code` 是展示用 Study Code，后端统一规范为 `01`-`99` 两位数字，新建时可省略并自动取最小可用编号。`DELETE` 为软删除，将状态改为 `deleted` 并保留历史数据。`terminated` 或 `deleted` Study 会拒绝患者、CRF、访视、随访、样本、组学、文件、质控、导出等业务写入。
...
- `patients.patient_number` 映射到 `PatientRecord.patientNumber`；`patients.name` 仅保留既有页面兼容展示，并由后端与自动生成的 `patient_number` 同步
