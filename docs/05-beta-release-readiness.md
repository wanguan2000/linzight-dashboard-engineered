...
本文记录从 `v0.0.1-beta.0` 到 `v1.0.5` 内部试点包的工程验证结果。当前结论是：项目适合发布为内部试点正式版本，用于客户在受控环境中按真实业务场景验证流程、角色、Study 配置和数据闭环；正式 Docker 启动为空库，仅保留首个 LZ 系统管理员。真实患者生产上线前仍必须完成生产级身份源、对象存储、PostgreSQL RLS、备份恢复演练和合规签字。

## 已完成检查

- 代码审查：检查前后端 Study 权限、`study_id` 隔离、CRF 版本、访视计划、随访记录、患者旅程聚合与静态导出链路。
- 后端 CRUD smoke：使用显式允许的临时 SQLite 数据库验证登录、患者创建/更新/删除、样本创建/更新/删除、组学创建/更新/删除、CRF 创建/更新、随访创建/更新、访视计划 upsert/update、Patient Journey 聚合、跨 Study 403 权限拒绝。
- 自动化 API smoke：`npm run smoke:api` 通过临时数据库验证 LZXK-01 Study 用户登录、患者查询隔离、跨 Study 403、Study 新建/终止/软删除、terminated Study 业务写入拒绝、用户创建/修改权限、Study 系统管理员分配、平台角色 Study scope 更新、CRF 字段更新、CRF 版本草稿/迁移审批/发布、提交人自审拒绝、execution logs 及审计、知情同意文件上传、样本/组学创建、PI 导出拒绝和数据管理员导出成功。
...
- Docker Compose：已提供 PostgreSQL、前端和后端镜像编排，后端首次启动会在 PostgreSQL volume 为空时初始化 schema 并只创建首个 LZ 系统管理员；`docker compose config` 用于校验编排文件语法。
- Docker 实测：Docker Desktop 启动后，`docker compose build` 和 `docker compose up -d` 已通过；后端容器首次启动为空库，仅创建首个 LZ 系统管理员，`GET /health` 与管理员 `POST /auth/login` 正常，前端容器通过 `http://localhost:5173/` 可访问。前端 Dockerfile 已复制 `resource/sle-crf-v0.1.schema.json`，避免容器内 TypeScript build 缺 CRF schema。
- 部署运维：已补 `docs/deployment-ops.md`，包括环境变量清单、Nginx 反向代理示例、Docker Compose 说明和旧 SQLite/上传目录备份恢复脚本。
...
- 配置检查：确认 `DATABASE_URL` / `LINZIGHT_DATABASE_URL` 已被后端连接函数实际读取；正式运行要求 PostgreSQL，临时 smoke 只有显式设置 `LINZIGHT_ALLOW_SQLITE_RUNTIME=1` 才可使用自定义 SQLite 路径运行验证。
...
- 后端 `connect()` 原先声明支持 `LINZIGHT_DATABASE_URL`，但实际总是连接默认 `backend/linzight_demo.db`；已修复为读取数据库 URL，并将默认运行时切换为 PostgreSQL。
...
- 配置与安装：Docker Compose PostgreSQL 功能测试环境、环境变量清单、Nginx 反向代理示例和旧 SQLite 备份恢复说明已完成；后续补生产数据库迁移、对象存储、正式备份策略和恢复演练。
- 数据层：正式运行已固定为 PostgreSQL；后续补 PostgreSQL RLS、托管备份和 schema 版本管理。
...
- 后端正式运行使用 PostgreSQL 和本地签名 token；真实患者生产上线前仍需集中身份源、托管密钥、PostgreSQL RLS、对象存储、备份恢复演练和合规签字。
