...
| Patient Queue | `/patients`, `/samples`, `/omics` through `fetchDemoDataset()` | Admin can filter all/LGL/RWD/LZXK; Study roles scoped | Patient rows expose Study ID; disease mix follows selected Study | Pass. LZXK-01 queue shows NSCLC/LUAD/LUSC/EGFR-TKI/ALK lung cohort only. |
